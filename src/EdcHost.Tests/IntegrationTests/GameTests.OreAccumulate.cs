...
public partial class GameTests
{
    ...
    public void OreAccumulationTests()
    {
        ...
        //Act2 Pave the way to accumulate iron 4~6
        //Invalid Placement
        ...
        //Act4 Pave way and Accumlate all Ore 1608~1692
        ...
        for(int i = 1; i < 9; i++)
        {
            game.Players[0].Trade(IPlayer.CommodityKindType.Wool);
            game.Tick();
            Assert.StrictEqual(i + 2, game.Players[0].WoolCount);
        }
        Assert.StrictEqual(0, game.Players[0].EmeraldCount);
        ...
        game.Tick();
        Assert.StrictEqual(7, game.Players[0].WoolCount);
        Assert.StrictEqual(64, game.Players[0].EmeraldCount);
        for(int i = 8; i <= MaximumItemCount; i++)
        {
            game.Players[0].Trade(IPlayer.CommodityKindType.Wool);
            game.Tick();
        }
        Assert.StrictEqual(MaximumItemCount, game.Players[0].WoolCount);
        Assert.False(game.Players[0].Trade(IPlayer.CommodityKindType.Wool));
        game.End();
    }
